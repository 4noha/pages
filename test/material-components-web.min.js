...
import $ from './jquery.js';
  ...
  deployButton.onclick = deployUnity;
  ...
  function uploadedImage(event) {
    ...
    reader.onload = function () {
      ...
      const canvas_b = imageUpload.parentElement.getElementsByClassName(
        "canvas-segmentation"
      )[1];
  ...
  function drawSegmentation(mask, targetElement) {
    ...
    const canvas_b = targetElement.getElementsByClassName(
      "canvas-segmentation"
    )[1];
    canvas.width = width;
    ...
    const ctx_b = canvas_b.getContext("2d");
    ctx_b.fillStyle = "#000000FF";
    ctx_b.fillRect(0, 0, width, height);
    ctx_b.fillStyle = "rgba(18, 181, 203, 0.7)";
  ...
  export async function deployUnity(event) {
    let img = canvas.toDataURL("image/png", 0.75);
    //sendImageBinary(img.substr(22));
    sendImageBinary(dataURItoBlob(img));
  }

  function sendImageBinary(bin) {
    const param = {
      method: "POST",
      headers: {
    ...
    //fetch("https://2qkjshogflodfputgmwnsbucfe0krsww.lambda-url.us-west-2.on.aws/", param)
    fetch("https://wt5s7zagbx3cncwlw6rinifwja0oygvn.lambda-url.us-west-2.on.aws/", param)
    .then((response)=>{
